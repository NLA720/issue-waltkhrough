...
async function patchOneIssue(containerId,issueId, payload) {
  try {
    let endpoint = config.endpoints.bim360Issues.patch_one_issue.format(containerId,issueId) 
    const headers = config.endpoints.httpHeaders(config.credentials.token_3legged)
    headers['Content-Type'] = 'application/json' 

    const response = await patch(endpoint, headers, payload);

    console.log(`patching one issue ${issueId} of container ${containerId}`)
    return response
     
  } catch (e) {
    console.error(`patching one issues ${issueId} of container ${containerId} failed: ${e}`)
    return {}
  }
}
...
async function getTypesandSubTypes(containerId,
  allTypes,
  offset,
  limit = 100) {
  try {
    ...
    if (response.results && response.results.length > 0) {
      ...
      response.results.forEach(t=>{
        allTypes = allTypes.concat(t);
      }) 
      if (response.pagination.totalResults > response.results.length) {
        offset += 100
        return getTypesandSubTypes(containerId, allTypes, offset);
      }
      else {
        return allTypes
      }
